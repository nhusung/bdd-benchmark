...
public:
  ...
  struct ngate
  {
    ...
    ngate(const type_t& ng_t, const std::vector<int>& lits)
      : ngate_type(ng_t)
      , lit_list(lits)
    {}

    ngate(const std::string& ng_t, const std::vector<int>& lits)
      : ngate_type(parse_type(ng_t))
      , lit_list(lits)
    {}
  ...
  struct quant_gate
  {
    ...
    {
#ifndef NDEBUG
      for (const int x : vars) { assert(0 <= x); }
#endif
    }
  ...
  class gate : private std::variant<const_gate, var_gate, ngate, ite_gate, quant_gate, output_gate>
  {
  public:
    using variant_t = std::variant<const_gate, var_gate, ngate, ite_gate, quant_gate, output_gate>;
...
private:
  ...
  __parse_qcir(std::istream& is)
  {
    ...
    { // Parse Prenex and Output
      const std::regex quant_regex("(" + kw_forall + "|" + kw_exists + ")" + ss + lparen + "("
                                   + gvar_list + ")" + rparen);
...
public:
  ...
  {
    ret_t res;
    while (begin != end) { res.push_back(find(*(begin++))); }
    return res;
  }
  ...
  gvar(int i) const noexcept
  {
    ...
    if (gvar_invmap_res == m_gvar_invmap.end()) { return ""; }
  ...
  size() const noexcept
  {
    return m_size - (at(const_idx[false]).refcount == 0u) - (at(const_idx[true]).refcount == 0u);
  }
...
public:
  ...
  {
    if (var.size() == 0) { throw std::invalid_argument("Cannot create var gate: ''"); }

    const int cached = find(var);
    if (cached != npos) { return cached; }

    const bool negated              = var.at(0) == '-';
    const std::string unnegated_var = negated ? var.substr(1) : var;

    if (unnegated_var.size() == 0) { throw std::invalid_argument("Cannot create var gate: '-'"); }
  ...
  //////////////////////////////////////////////////////////////////////////////
  int
  add_ngate(const std::string gvar, const ngate::type_t& ng_t, const std::vector<int>& lits)
  {
    if (m_has_output_gate) {
      throw std::invalid_argument(
        "Cannot create an NGATE gate after having created the OUTPUT gate");
    }
  ...
  int
  add_ngate(const std::string gvar, const std::string& ng_t, const std::vector<int>& lits)
  {
    return add_ngate(gvar, ngate::parse_type(ng_t), lits);
  }

  int
  add_ngate(const std::string gvar, const ngate::type_t& ng_t, const std::vector<std::string>& lits)
  {
  ...
  int
  add_ite_gate(const std::string gvar, const Lits& lits)
  {
    if (m_has_output_gate) {
  ...
  int
  add_ite_gate(const std::string gvar, const std::vector<std::string> lits)
  {
    return add_ite_gate(gvar, find(lits.begin(), lits.end()));
  }

  int
  add_ite_gate(const std::string gvar, const int g_if, const int g_then, const int g_else)
  {
               ...
               const std::string g_else)
  {
    return add_ite_gate(
      gvar,
      std::array<int, 3>{ __find_or_add(g_if), __find_or_add(g_then), __find_or_add(g_else) });
  }
  ...
  int
  add_quant_gate(const quant_gate::type_t Q, IT vars_begin, IT vars_end, int i)
  {
    ...
    // --------------------------
    // Case: Empty var-list
    if (vars_begin == vars_end) { return i; }
    ...
    while (vars_begin != vars_end) {
      ...
      if (quant_var_idx == npos) {
        std::cerr << "Skipping variable '" << quant_var << "' that never has been mentioned"
                  << std::endl;
        continue;
      }
      if (quant_var_idx < 0) {
      ...
      if (!g.is<var_gate>()) {
        throw std::invalid_argument("Quantified variable '" + quant_var
                                    + "' refers to a non-var gate");
      }
  ...
  template <typename IT>
  int
  add_quant_gate(const quant_gate::type_t Q, IT vars_begin, IT vars_end, std::string i)
  {
    return add_quant_gate(Q, vars_begin, vars_end, find(i));
  }

  int
  add_quant_gate(const quant_gate::type_t Q, std::vector<std::string> vars, int i)
  {
    return add_quant_gate(Q, vars.begin(), vars.end(), i);
  }

  int
  add_quant_gate(const quant_gate::type_t Q, std::vector<std::string> vars, std::string i)
  {
  ...
  {
    if (m_has_output_gate) { throw std::invalid_argument("Cannot create two OUTPUT gates"); }
    ...
    if (deref_unreachable) {
      ...
      while (g_idx < res_idx) {
        int next_reachable = it != reachable_indices.cend() ? *(it++) : res_idx;
        ...
        while (g_idx < next_reachable) {
          ...
          __at(g_idx++).match(
            ...
            [this](const quant_gate& g) -> void { __dec_refcount(__at(g.lit)); },
...
public:
           ...
           const int root_idx) const
  {
    ...
    while (!dfs.empty()) {
      ...
      g.match(
        [&push_if_unvisited](const ngate& g) -> void {
          if constexpr (rtl) {
            for (auto g_it = g.lit_list.cbegin(); g_it != g.lit_list.cend(); ++g_it) {
        ...
        [&push_if_unvisited](const ite_gate& g) -> void {
          push_if_unvisited(g.lits[rtl ? 0 : 2]);
          push_if_unvisited(g.lits[rtl ? 1 : 1]);
          push_if_unvisited(g.lits[rtl ? 2 : 0]);
        },
        [&push_if_unvisited](const quant_gate& g) -> void { push_if_unvisited(g.lit); },
        [&push_if_unvisited](const output_gate& g) -> void { push_if_unvisited(g.lit); },
        [](const auto& /*g*/) -> void { /* do nothing */ });
    }
  }
...
private:
  ...
  {
    if (g.refcount == 0u) { throw std::underflow_error("Trying to decrement refcount below 0."); }
    g.refcount--;
  }
  ...
  __push_gate(const size_t d, const Gate& g)
  {
    ...
    if constexpr (!std::is_same<Gate, const_gate>::value) { m_roots++; }
  ...
  {
    ret_t res;
    while (begin != end) { res.push_back(__find_or_add(*(begin++))); }
    return res;
  }
};
...
public:
  void
  add(const int qcir_variable, const int dd_variable)
  {
...
{
  var_order_map vom;
  for (size_t x = 0; x < q.vars(); ++x) { vom.add(x, x); }
  return vom;
}
...
obtain_var_order__level(const qcir& q)
{
  ...
  for (size_t x = 0; x < q.vars(); ++x) { var_depth.insert({ x, q.depth() }); }

  const auto dec_if_var = [&q, &var_depth](const int lit, size_t depth) {
    q.at(lit).match(
      [&var_depth, &depth](const qcir::var_gate& g) -> void {
  ...
  for (auto g_it = q.cbegin(); g_it != q.cend(); ++g_it) {
    ...
    g.match(
      [&dec_if_var, &depth](const qcir::ngate& g) -> void {
        for (auto g_it = g.lit_list.cbegin(); g_it != g.lit_list.cend(); ++g_it) {
          dec_if_var(*g_it, depth);
        }
      },
      [&dec_if_var, &depth](const qcir::ite_gate& g) -> void {
        dec_if_var(g.lits[0], depth);
        dec_if_var(g.lits[1], depth);
        dec_if_var(g.lits[2], depth);
      },
      [&var_depth, &depth](const qcir::quant_gate& g) -> void {
      ...
      [&dec_if_var, &depth](const qcir::output_gate& g) -> void { dec_if_var(g.lit, depth); },
      [](const auto& /*g*/) -> void { /* do nothing */ });
  }
  ...
  std::vector<int> vars;
  for (size_t x = 0; x < q.vars(); ++x) { vars.push_back(x); }
  std::sort(vars.begin(), vars.end(), [&var_depth, &tie_breaker](const int a, const int b) -> bool {
  ...
  var_order_map res;
  for (size_t x = 0; x < q.vars(); ++x) { res.add(vars[x], x); }
  return res;
}
...
{
  switch (vo) {
  case INPUT: return obtain_var_order__input(q);
  case LEVEL: return obtain_var_order__level(q);
  case DF_RTL: return obtain_var_order__df<true>(q);
  default:
  case DF_LTR: return obtain_var_order__df<false>(q);
  }
}
...
{
  exe_order res = q.reachable();
  std::sort(res.begin(), res.end(), [&q](const int a, const int b) -> bool {
    if (q.at(a).depth != q.at(b).depth) { return q.at(a).depth < q.at(b).depth; }
    return a < b;
  });
  return res;
}
...
{
  switch (exe_order) {
  case INPUT: return obtain_exe_order<INPUT>(q);
  default:
  case DF_LTR:
  case DF_RTL:
  case LEVEL: return obtain_exe_order<LEVEL>(q);
  }
}
...
// Max Index
int
max_solve_idx(const qcir& q)
{
  bool run                            = true;
  qcir::quant_gate::type_t root_quant = qcir::quant_gate::EXISTS;

  int res = q.root_idx();
  q.at(res--).match([&run](const qcir::output_gate& /*g*/) { run = false; },
                    [&root_quant](const qcir::quant_gate& g) { root_quant = g.quant; },
                    [](const auto& /*g*/) -> void { /* do nothing */ });
  ...
  while (run && res > 2) {
    ...
    g.match([&run](const qcir::output_gate& /*g*/) { run = false; },
            [&run, &root_quant](const qcir::quant_gate& g) {
...
solve_res
solve(Adapter& adapter, qcir& q, const variable_order vo = variable_order::INPUT)
{
  ...
  for (auto exo_it = exo.cbegin(); exo_it != exo.cend(); ++exo_it) {
    ...
    const typename Adapter::dd_t g_dd =
      g.match([&adapter](const qcir::const_gate& g) ->
              ...
              [&cache_get](const qcir::ngate& g) -> typename Adapter::dd_t {
                const auto apply =
                  [&g](const typename Adapter::dd_t& dd_1,
                    ...
                    switch (g.ngate_type) {
                    case qcir::ngate::AND: return dd_1 & dd_2;
                    case qcir::ngate::OR: return dd_1 | dd_2;
                    case qcir::ngate::XOR: return dd_1 ^ dd_2;
                    }
                ...
                { // Populate FIFO queue with pairs of BDDs
                  ...
                  while (g_it != g.lit_list.cend()) {
                    ...
                    if (g_it == g.lit_list.cend() && tmp.empty()) { return apply(dd_1, dd_2); }
                ...
                while (true) {
                  const auto dd_1 = tmp.front();
                  tmp.pop();
                  const auto dd_2 = tmp.front();
                  tmp.pop();

                  if (tmp.empty()) { return apply(dd_1, dd_2); }
                  tmp.push(apply(dd_1, dd_2));
                }
              },
              [&adapter, &cache_get](const qcir::ite_gate& g) -> typename Adapter::dd_t {
              ...
              [&adapter, &vom, &cache_get](const qcir::quant_gate& g) -> typename Adapter::dd_t {
                std::set<int> vars;
                ...
                */
                ...
                return g.quant == qcir::quant_gate::EXISTS ? adapter.exists(cache_get(g.lit), pred)
                                                           : adapter.forall(cache_get(g.lit), pred);
              },
              [&cache_get, &t_prenex_before](const qcir::output_gate& g) -> typename Adapter::dd_t {
                t_prenex_before = now();
  ...
  return { sat_res,
           ...
           { duration_ms(t_prenex_before, t_solve_after),
             ...
             { cache_max_size },
             { dd_max_size, dd_matrix_max_size, dd_prenex_max_size } } };
}
