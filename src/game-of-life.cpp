...
public:
  ...
  cell(char row, char col, bool prime = prime::pre)
    ...
    , _prime(prime)
  {
    if (this->out_of_range()) { throw std::out_of_range("Cell not within valid boundaries"); }
  }
...
public:
  ...
  bool
  in_neighbourhood(const cell& o) const
  {
  ...
  neighbourhood() const
  {
    ...
    std::vector<cell> res = { cell(this->row() - 1, this->col() - 1, prime::pre),
    ...
    for ([[maybe_unused]] const cell& c : res) { assert(!c.out_of_range()); }
  ...
  bool
  is_neighbour(const cell& o) const
  {
    if (this->row() == o.row() && this->col() == o.col()) { return false; }
...
public:
    ...
    : _sym(s)
  {
      ...
      for (int row = MIN_ROW(prime::pre); row <= MAX_ROW(prime::pre); ++row) {
        for (int left_col = MIN_COL(prime::pre); left_col <= mid_col; ++left_col) {
          ...
          if (!post_left.out_of_range()) {
            ...
            if (add_mirror) { this->_map.insert({ cell(pre_right, prime::post), post_var }); }
          }
        }
      }
      break;
      ...
      for (int row = MIN_ROW(prime::pre); row <= MAX_ROW(prime::pre); ++row) {
        ...
        for (int col = MIN_COL(prime::pre); col <= max_col; ++col) {
          ...
          if (!post.out_of_range()) {
            ...
            if (add_mirror) { this->_map.insert({ cell(pre_mirror, prime::post), post_var }); }
    ...
    case symmetry::rotate_90: {
      if (!is_square()) {
        throw std::invalid_argument(
          "Rotational symmetry (90 degrees) is only available for square grids.");
      }
      ...
      for (int tl_row = mid_row; MIN_ROW(prime::pre) <= tl_row; --tl_row) {
        for (int tl_col = mid_col; MIN_COL(prime::pre) <= tl_col; --tl_col) {
          ...
          if (!cell(pre_tl, prime::post).out_of_range()) {
            ...
            if (add_tr) { this->_map.insert({ cell(pre_tr, prime::post), post_var }); }
            if (add_bl) { this->_map.insert({ cell(pre_bl, prime::post), post_var }); }
            if (add_br) { this->_map.insert({ cell(pre_br, prime::post), post_var }); }
          }
        }
      }
      break;
      ...
      for (int top_row = mid_row; MIN_ROW(prime::pre) <= top_row; --top_row) {
        for (int top_col = MIN_COL(prime::pre); top_col <= MAX_COL(prime::pre); ++top_col) {
          ...
          if (!post_top.out_of_range()) {
            ...
            if (add_bot) { this->_map.insert({ cell(pre_bot, prime::post), post_var }); }
          }
        }
      }
      break;
    ...
    for (const auto& kv : this->_map) { this->_inv.at(kv.second) = kv.first; }
  }

  /// \brief Obtain variable for a cell.
  int
  var_from_cell(const cell& c) const
  {
    if (c.out_of_range()) { throw std::out_of_range("Cell not within valid boundaries"); }

    const auto res = this->_map.find(c);

    if (res == this->_map.end()) { throw std::out_of_range("Cell not found in 'cell -> var' map"); }
    return res->second;
  }
  ...
  cell
  cell_from_var(const int x) const
  {
  ...
  cell
  cell_from_var(const int x, const cell& c) const
  {
  ...
  bool
  row_symmetric(const cell& c, const int row) const
  {
...
construct_count(Adapter& adapter, const var_map& vm, const cell& c, const int alive)
{
  ...
  if (alive > remaining_cells) { return adapter.bot(); }
  ...
  for (int x = vm.varcount() - 1; 0 <= x; --x) {
    ...
    if (curr_cell.prime() == prime::pre && c.in_neighbourhood(curr_cell)) {
      ...
      // of true variables.
      alive_max = 0 < remaining_cells && remaining_cells == alive_max ? alive_max - 1 : alive_max;
...
construct_eq(Adapter& adapter, const var_map& vm, const cell& c)
{
  ...
  // below 'x_post'
  for (; x_post < x; --x) { root0 = adapter.build_node(x, root0, root0); }
  ...
  for (; 0 <= x; --x) { root0 = adapter.build_node(x, root0, root0); }
...
template <typename Adapter>
typename Adapter::dd_t
acc_rel(Adapter& adapter, const var_map& vm, const cell& c)
{
...
template <typename Adapter>
typename Adapter::dd_t
acc_rel(Adapter& adapter, const var_map& vm, const int row)
{
...
template <typename Adapter>
typename Adapter::dd_t
acc_rel(Adapter& adapter, const var_map& vm, const bool bottom)
{
